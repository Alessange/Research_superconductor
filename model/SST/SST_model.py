...
import numpy as np
from data_processing.Gen_atom import atom
...
def word_pos_embedding(data):
    ...
    for sentence in data:
        ...
        combined_token = np.array(tokens + pad_atom)
        combined_pos = np.array(positions + pad_pos)
        tokens_1 = torch.tensor(combined_token,dtype=torch.float32)
        positions_1 = torch.tensor(combined_pos,dtype=torch.float32)

        tokens_1 = tokens_1.unsqueeze(0) # Add batch dimension
        positions_1 = positions_1.unsqueeze(0) # Add batch dimension

        sentence_embed = operation(values=tokens_1, keys=tokens_1, query=positions_1, mask=None)
...
class Transformer(nn.Module):
    def __init__(self, d_model, nhead, nhid, nlayers, dropout=0.1,
        ...
        self.transformer_decoder = nn.TransformerDecoder(decoder_layer,
                                                         nlayers).to(
            self.device)
        self.output_layer = regressoionHead(25*2)
    ...
    def generate_padding_mask(self, src):
        ...
        for sentence in src:
            sent_mask = []
            # step over by 2, as
            # we are treating (Atom, Position) as a single unit
    ...
    def forward(self, src, tgt):
        ...
        # The mask should have the same size as the source tensor,
        # but with 0s in padding positions and 1s elsewhere
        src_mask = (src_mask == 1)
...
if __name__ == '__main__':
    path1 = "/Users/huangshixun/Desktop/Research_superconductor/data/pre_train_new.json"
